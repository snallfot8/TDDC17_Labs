...
//		if (iteration >= 100000 && iteration < 101000) {
//			explore = false;
//		}
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				if (Qtable.get(prev_stateaction) == null) {
					...
					double newValue = Qtable.get(prev_stateaction) + alpmha(Ntable.get(prev_stateaction))*(previous_reward+
							GAMMA_DISCOUNT_FACTOR*getMaxActionQValue(new_state)-Qtable.get(prev_stateaction));
					Qtable.put(prev_stateaction, newValue);			
				}
				
				//Math.abs(rand.nextDouble()) < GAMMA_DISCOUNT_FACTOR
